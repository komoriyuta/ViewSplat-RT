...
class CrossAttention(nn.Module):
    ...
    def forward(self, query, key, value, qpos, kpos, mask=None):
        B, Nq, C = query.shape
        Nk = key.shape[1]
        Nv = value.shape[1]
        ...
        # q torch.Size([1, 12, 257, 64]) torch.Size([1, 12, 514, 64]) torch.Size([1, 12, 514, 64]), 12 for the num_heads
        if self.rope is not None:
            q = self.rope(q, qpos)
            k = self.rope(k, kpos)
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, y, xpos, ypos, mask=None):
        # print("x", x.shape, y.shape, xpos.shape, ypos.shape, mask.shape)
        # x torch.Size([b, 257, 768]) torch.Size([b, 514, 768]) torch.Size([b, 257, 2]) torch.Size([b, 514, 2])
        x = x + self.drop_path(self.attn(self.norm1(x), xpos))
        y_ = self.norm_y(y)
        x = x + self.drop_path(self.cross_attn(self.norm2(x), y_, y_, xpos, ypos, mask))
